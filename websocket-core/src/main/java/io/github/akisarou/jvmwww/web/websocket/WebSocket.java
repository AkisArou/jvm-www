...
public final class WebSocket extends EventTarget implements WebSocketTransportListener, EventListener, RuntimeTask {
    ...
    public WebSocket(RuntimeInstance runtime, WebSocketTransport transport, String url, String protocol) {
        this(runtime, transport, url, new String[] {Objects.requireNonNull(protocol, "protocol")});
    }
    public WebSocket(RuntimeInstance runtime, WebSocketTransport transport, String input, String[] protocols) {
    ...
    public WebSocket(RuntimeInstance runtime, WebSocketTransport transport, URL input, String[] protocols) {
        super(checkedRuntime(runtime));
        URL checked = Objects.requireNonNull(input, "url");
        if (checked.getRuntime() != runtime) throw new IllegalArgumentException("WebSocket URL belongs to another RuntimeInstance");
    ...
    public int getReadyState() { assertAccess(); return readyState; }
    public long getBufferedAmount() {
        assertAccess();
        if (readyState != CLOSED && !isBufferSampleQueued()) refreshBufferedAmountFromTransport(true);
        return bufferedAmount;
    }
    public String getExtensions() { assertAccess(); return extensions; }
    public String getProtocol() { assertAccess(); return protocol; }
    public String getBinaryType() { assertAccess(); return binaryType; }
    ...
    public EventListener getOnOpen() { assertAccess(); return onOpen; }
    public EventListener getOnMessage() { assertAccess(); return onMessage; }
    public EventListener getOnError() { assertAccess(); return onError; }
    public EventListener getOnClose() { assertAccess(); return onClose; }
    public void setOnOpen(EventListener listener) { assertAccess(); onOpen = setHandler(OPEN_EVENT_TYPE, onOpen, listener); }
    public void setOnMessage(EventListener listener) { assertAccess(); onMessage = setHandler(MESSAGE_EVENT_TYPE, onMessage, listener); }
    public void setOnError(EventListener listener) { assertAccess(); onError = setHandler(ERROR_EVENT_TYPE, onError, listener); }
    public void setOnClose(EventListener listener) { assertAccess(); onClose = setHandler(CLOSE_EVENT_TYPE, onClose, listener); }
    ...
    public void send(String data) {
        ...
        if (!canSendToTransport()) { addNonTransportBufferedAmount(size); return; }
        WebSocketTransportCall local = requireCall();
        boolean accepted = false;
        try { accepted = local.sendText(scalar); } catch (Throwable error) { rethrowIfFatal(error); }
        finishSend(accepted, size);
    }
    public void send(byte[] data) { send(data, 0, Objects.requireNonNull(data, "data").length); }
    public void send(byte[] data, int offset, int length) {
        ...
        if (readyState == CONNECTING) throw DOMException.invalidState("WebSocket is connecting");
        prepareBufferedAmountBaseline();
        if (!canSendToTransport()) { addBufferedAmount(length); addNonTransportBufferedAmount(length); return; }
    ...
    public void send(Blob data) {
        ...
        if (checked.getRuntime() != getRuntime()) throw new IllegalArgumentException("WebSocket Blob belongs to another RuntimeInstance");
        if (readyState == CONNECTING) throw DOMException.invalidState("WebSocket is connecting");
        long size = checked.getSize();
        prepareBufferedAmountBaseline();
        if (!canSendToTransport()) { addBufferedAmount(size); addNonTransportBufferedAmount(size); return; }
        BufferedBodySnapshot snapshot = checked.snapshot();
        addBufferedAmount(size);
        sendBinarySnapshot(snapshot, size);
    }

    public void close() { closeInternal(0, "", false); }
    public void close(int code) { closeInternal(code, "", true); }
    public void close(int code, String reason) { closeInternal(code, reason, true); }
    ...
    public void onOpen(String selectedProtocol, String selectedExtensions) {
        ...
        if (!checkedProtocol.isEmpty() && !wasProtocolOffered(checkedProtocol)) {
            enqueueEvent(CloseEvent.failure(), true);
            cancelCurrentCall();
            return;
        }
    ...
    @Override public void onBinaryMessage(byte[] bytes) {
        byte[] owned = Objects.requireNonNull(bytes, "bytes");
        if (!canPublishNonTerminal()) return;
        enqueueEvent(MessageEvent.binary(owned, origin), false);
    }
    @Override public void onClosing() { enqueueClosing(); }
    @Override public void onClosed(int code, String reason, boolean wasClean) {
        enqueueEvent(CloseEvent.transportClose(code, reason, wasClean), true);
    }
    ...
    public void execute(RuntimeInstance runtime) {
        if (runtime != getRuntime()) throw new IllegalArgumentException("WebSocket delivered by another RuntimeInstance");
        QueuedWebSocketEvent event;
        synchronized (this) {
            if (discarded) return;
            event = eventHead;
            if (event == null) { taskActive = false; return; }
            eventHead = event.nextWebSocketEvent;
            if (eventHead == null) eventTail = null;
            event.nextWebSocketEvent = null;
        }
        try { event.deliver(this); }
        finally {
            boolean reschedule;
            synchronized (this) {
                if (discarded || eventHead == null) { taskActive = false; reschedule = false; }
    ...
    String getBinaryTypeInternal() { return binaryType; }
    void deliverMessage(MessageEvent event) { if (readyState == OPEN) dispatchEvent(event); }
    void deliverClose(CloseEvent event, boolean fireErrorFirst) {
        ...
        readyState = CLOSED;
        synchronized (this) { closingQueued = true; terminalQueued = true; call = null; }
    ...
    private void start(WebSocketTransport transport) {
        try {
            ...
            boolean cancel;
            synchronized (this) { call = started; cancel = discarded || terminalQueued; }
            if (cancel) cancelQuietly(started);
        } catch (Throwable error) { rethrowIfFatal(error); onFailure(error); }
    }
    private void sendBinarySnapshot(BufferedBodySnapshot snapshot, long size) {
        WebSocketTransportCall local = requireCall();
        boolean accepted = false;
        try { accepted = local.sendBinary(snapshot); } catch (Throwable error) { rethrowIfFatal(error); }
        finishSend(accepted, size);
    }
    private void finishSend(boolean accepted, long size) {
        if (accepted) { scheduleBufferSample(); return; }
    ...
    private void failConnectionFromOwner() {
        ...
        readyState = CLOSING;
        WebSocketTransportCall local;
        synchronized (this) { closingQueued = true; local = call; }
        cancelQuietly(local);
        enqueueEvent(CloseEvent.failure(), true);
    }
    private void closeInternal(int code, String reason, boolean codePresent) {
        assertAccess();
        String checkedReason = codePresent ? WebSocketScalar.fromString(reason == null ? "" : reason, "reason") : "";
        if (codePresent && code != 1000 && (code < 3000 || code > 4999)) {
            throw new DOMException("WebSocket close code must be 1000 or between 3000 and 4999", "InvalidAccessError");
        }
        if (Utf8Codec.encodedLength(checkedReason) > 123) throw new DOMException("WebSocket close reason exceeds 123 UTF-8 bytes", "SyntaxError");
        ...
        readyState = CLOSING;
        boolean accepted = false;
        try { accepted = local.close(codePresent ? code : 0, checkedReason); } catch (Throwable error) { rethrowIfFatal(error); }
        if (!accepted) { cancelQuietly(local); enqueueEvent(CloseEvent.failure(), true); }
    }
    private void deliverOpen(OpenEvent event, String selectedProtocol, String selectedExtensions) {
    ...
    private void deliverClosing() { if (readyState == CONNECTING || readyState == OPEN) readyState = CLOSING; }
    private void deliverBufferSample() {
        synchronized (this) { bufferSampleQueued = false; }
    ...
    private void refreshBufferedAmountFromTransport(boolean failOnError) {
        WebSocketTransportCall local;
        synchronized (this) { local = call; }
        if (local == null) return;
        try {
            long queued = local.getQueuedByteCount();
            if (queued < 0L) throw new IllegalStateException("WebSocket transport returned a negative queued-byte count");
    ...
    private boolean canSendToTransport() {
        ...
        synchronized (this) { return !closingQueued && !terminalQueued && call != null; }
    }
    private boolean canPublishNonTerminal() {
        synchronized (this) { return !discarded && !closingQueued && !terminalQueued; }
    }
    private boolean isBufferSampleQueued() { synchronized (this) { return bufferSampleQueued; } }
    private void enqueueOpen(OpenEvent event) {
        ...
        synchronized (this) {
            ...
            if (!taskActive) { taskActive = true; admit = true; }
        }
    ...
    private void enqueueEvent(QueuedWebSocketEvent event, boolean terminal) {
        boolean admit = false;
        synchronized (this) {
            if (discarded || terminalQueued) { event.discardPayload(); return; }
            if (terminal) { terminalQueued = true; closingQueued = true; }
            appendEventLocked(event);
            if (!taskActive) { taskActive = true; admit = true; }
        }
        if (admit) getRuntime().admitHostTask(this);
    ...
    private boolean wasProtocolOffered(String value) {
        for (String offered : offeredProtocols) if (offered.equals(value)) return true;
        return false;
    }
    private WebSocketTransportCall requireCall() {
        synchronized (this) {
            if (call == null) throw new IllegalStateException("WebSocket transport call is unavailable");
            return call;
        }
    }
    ...
    private void addBufferedAmount(long amount) { bufferedAmount = saturatingAdd(bufferedAmount, amount); }
    private void addNonTransportBufferedAmount(long amount) { nonTransportBufferedAmount = saturatingAdd(nonTransportBufferedAmount, amount); }
    private void assertAccess() { WebSocketRuntimeChecks.assertLanguageExecution(getRuntime()); }
    ...
    private static String[] validateProtocols(String[] protocols) {
        ...
        for (int index = 0; index < result.length; index++) {
            ...
            if (protocol.isEmpty()) throw syntaxError("WebSocket protocol must not be empty");
            for (int offset = 0; offset < protocol.length(); offset++) if (!isTokenCharacter(protocol.charAt(offset))) throw syntaxError("Invalid WebSocket protocol: " + protocol);
            for (int previous = 0; previous < index; previous++) if (result[previous].equals(protocol)) throw syntaxError("Duplicate WebSocket protocol: " + protocol);
        }
        return result;
    }
    private static WebSocketUrl parseUrl(RuntimeInstance runtime, String input) {
        ...
        final URL parsed;
        try { parsed = new URL(runtime, mapped); }
    ...
    private static WebSocketUrl parseCanonicalUrl(URL parsed) {
        ...
        if (href.indexOf('#') >= 0) throw syntaxError("WebSocket URL must not contain a fragment");
        if (!parsed.getUsername().isEmpty() || !parsed.getPassword().isEmpty()) throw syntaxError("WebSocket URL must not contain credentials");
        String protocol = parsed.getProtocol();
        if ("http:".equals(protocol)) return new WebSocketUrl(replaceScheme(href, "ws"), replaceScheme(parsed.getOrigin(), "ws"));
        if ("https:".equals(protocol)) return new WebSocketUrl(replaceScheme(href, "wss"), replaceScheme(parsed.getOrigin(), "wss"));
    ...
    private static String preprocessUrlInput(String input) {
        ...
        if (!removed) return start == 0 && end == input.length() ? input : input.substring(start, end);
        char[] output = new char[kept];
        int written = 0;
        for (int index = start; index < end; index++) {
            char current = input.charAt(index);
            if (current != '\t' && current != '\n' && current != '\r') output[written++] = current;
        }
        return new String(output);
    }
    private static DOMException syntaxError(String message) { return new DOMException(message, "SyntaxError"); }
    ...
    private static void checkRange(int size, int offset, int length) {
        if (offset < 0 || length < 0 || offset > size - length) throw new IndexOutOfBoundsException("offset=" + offset + ", length=" + length + ", size=" + size);
    }
    ...
    private static void cancelQuietly(WebSocketTransportCall local) {
        if (local == null) return;
        try { local.cancel(); } catch (Throwable error) { rethrowIfFatal(error); }
    }
    ...
    private static final class WebSocketUrl {
        final String url;
        final String origin;
        WebSocketUrl(String url, String origin) { this.url = url; this.origin = origin; }
    }
    private static final class OpenEvent extends QueuedWebSocketEvent {
        ...
        @Override void deliver(WebSocket socket) { socket.deliverOpen(this, selectedProtocol, selectedExtensions); }
    }
    private static final class ClosingEvent extends QueuedWebSocketEvent {
        ClosingEvent() { super("websocket-closing"); }
        @Override void deliver(WebSocket socket) { socket.deliverClosing(); }
    }
    private static final class BufferSampleEvent extends QueuedWebSocketEvent {
        BufferSampleEvent() { super("websocket-buffer-sample"); }
        @Override void deliver(WebSocket socket) { socket.deliverBufferSample(); }
    }
}
