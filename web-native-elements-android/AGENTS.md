...
- Do not retain or expose `android.view.View`, `android.graphics.Matrix`, renderer shadow nodes, JNI handles, platform rectangles, sparse maps, generic collections, or child snapshots.
...
Run `./scripts/test-native-elements-android.sh`, `./scripts/test-native-element-relations.sh`, and `./scripts/test-native-element-collections.sh` before every change.
